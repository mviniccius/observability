    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">{alunos.length} aluno{alunos.length !== 1 ? 's' : ''} cadastrado{alunos.length !== 1 ? 's' : ''}</p>
        </div>
        <button
          onClick={openCreate}
          className="flex items-center gap-2 bg-primary hover:bg-primary-dark text-white px-4 py-2.5 rounded-lg text-sm font-medium transition-colors cursor-pointer"
        >
        ...
        <div className="px-6 py-4 border-b border-slate-100">
          <div className="relative max-w-sm">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 border border-slate-200 rounded-lg text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-colors"
            />
          </div>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 text-left">
                {['Nome', 'CPF', 'E-mail', 'Telefone', 'Cidade / UF', 'Nascimento', 'Ações'].map(h => (
              ...
              {loading ? (
                ...
                paginated.map(aluno => (
                  <tr key={aluno.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-slate-600 font-mono text-xs">{aluno.cpf}</td>
                    <td className="px-6 py-4 text-slate-600">{aluno.email}</td>
                    <td className="px-6 py-4 text-slate-600">{aluno.telefone || '—'}</td>
                    <td className="px-6 py-4 text-slate-600">
                      {aluno.cidade ? `${aluno.cidade} / ${aluno.estado}` : '—'}
                    </td>
                    <td className="px-6 py-4 text-slate-600">{formatDate(aluno.data_nascimento)}</td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => openEdit(aluno)}
                          className="p-1.5 rounded-lg hover:bg-blue-50 text-slate-400 hover:text-primary transition-colors cursor-pointer"
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 bg-red-500 hover:bg-red-600 text-white rounded-lg py-2.5 text-sm font-medium transition-colors cursor-pointer"
              >
                Remover
              </button>
            </div>
